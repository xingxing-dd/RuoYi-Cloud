<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="收款人账号" prop="payeeAcct">
        <el-input
          v-model="queryParams.payeeAcct"
          placeholder="请输入收款人账号"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="收款币种" prop="payeeCurrency">
        <el-input
          v-model="queryParams.payeeCurrency"
          placeholder="请输入收款币种"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['recharge:bank:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['recharge:bank:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="bankList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="swift" align="center" prop="swift" />
      <el-table-column label="收款人名称" align="center" prop="payeeName" />
      <el-table-column label="收款人账号" align="center" prop="payeeAcct" />
      <el-table-column label="收款币种" align="center" prop="payeeCurrency">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.recharge_currencies" :value="scope.row.payeeCurrency"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="swift" prop="swift">
          <el-input v-model="form.swift" placeholder="请输入swift" />
        </el-form-item>
        <el-form-item label="收款人名称" prop="payeeName">
          <el-input v-model="form.payeeName" placeholder="请输入收款人名称" />
        </el-form-item>
        <el-form-item label="收款人账号" prop="payeeAcct">
          <el-input v-model="form.payeeAcct" placeholder="请输入收款人账号" />
        </el-form-item>
        <el-form-item label="收款币种" prop="payeeCurrency">
          <el-input v-model="form.payeeCurrency" placeholder="请输入收款币种" />
        </el-form-item>
        <el-form-item label="是否展示" prop="status">
...
<script>
import { listBank, getBank, delBank, addBank, updateBank } from "@/api/recharge/bank";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        swift: [
          { required: true, message: "swift不能为空", trigger: "blur" }
        ],
        payeeName: [
        ...
        payeeCurrency: [
          { required: true, message: "收款币种不能为空", trigger: "blur" }
        ],
        status: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除充值银行信息编号为"' + ids + '"的数据项？').then(function() {
        return delBank(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('recharge/bank/export', {
        ...this.queryParams
