<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tradeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="完成时间" align="center" prop="completeAt" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.completeAt, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="交割时间" align="center" prop="deliveryAt" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.deliveryAt, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
